...
class CascadeMVSNet(nn.Module):
    def __init__(self, cr_in_chs, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        if self.share_cr:
            self.cost_regularization = CostRegNet(in_channels=cr_in_chs, base_channels=8)
        else:
            self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=cr_in_chs[i],
    ...
    def forward(self, imgs, view_feats, proj_matrices, depth_values):
        """
        imgs: (B, V, L, C, H, W)
        view_feats: dict of list, len=v, (b, c, h, w)
        proj_matrices: dict, (b, v, 4, 4)
        depth_values: (b, ndepth)
        """
        b, v, l, _, h, w = imgs.shape
        ...
        for stage_idx in range(self.num_stage):
            features_stage = view_feats["stage{}".format(stage_idx + 1)]  # list, len=v, (b, c, h, w)
            proj_matrices_stage = proj_matrices["stage{}".format(stage_idx + 1)]  # (b, v, 4, 4)
            ...
            if depth is not None:
                ...
                cur_depth = depth_values  # (b, ndepth)
            ...
            outputs_stage = self.DepthNet(features_stage, proj_matrices_stage,
                                          depth_values=F.interpolate(depth_range_samples.unsqueeze(1),
                                                                     [self.ndepths[stage_idx], h//int(stage_scale), w//int(stage_scale)], mode='trilinear',
                                                                     align_corners=Align_Corners_Range).squeeze(1),
...
def cas_mvpsnet_loss(inputs, depth_gt_ms, mask_ms, depth_loss_weights=None):
    loss = {}
    ...
    loss['total_depth_loss'] = total_loss
    return loss
